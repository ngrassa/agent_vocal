...
SIP_EXT_PASS="${SIP_EXTENSION_PASS:-restaurant2024}"
PUBLIC_IP="${PUBLIC_IP:-}"
...
; endpoint nommé = SIP username → identify_by=username fait le match
[${SIP_EXTENSION}]
type=endpoint
context=restaurants
disallow=all
...
# ── SIP trunk (optionnel — si configuré) ──────────────────────────
...
; Extension interne (test Zoiper : composer le 7000)
[restaurants]
exten => 7000,1,NoOp(Test agent restaurant)
 same => n,Answer()
 same => n,AGI(restaurant_agi.py)
 same => n,Hangup()
...
# ── Services persistants (Whisper STT + Piper TTS) ────────────────
echo "Démarrage Whisper STT (small)..."
python3 /usr/share/asterisk/agi-bin/whisper_service.py &

echo "Démarrage Piper TTS (fr_FR féminin)..."
python3 /usr/share/asterisk/agi-bin/piper_service.py &

# ── Attendre que les deux services soient prêts ───────────────────
echo "Attente des services..."
for i in $(seq 1 90); do
    W=$(curl -sf http://127.0.0.1:5001/ 2>/dev/null && echo 1 || echo 0)
    P=$(curl -sf http://127.0.0.1:5002/ 2>/dev/null && echo 1 || echo 0)
    if [ "$W" = "1" ] && [ "$P" = "1" ]; then
        echo "Whisper + Piper prêts (${i}s)"
        break
    fi
    sleep 2
done

# ── Pré-génération des fichiers audio statiques via Piper ─────────
echo "Génération des fichiers audio statiques..."
python3 - <<'PYEOF'
import os, subprocess, json, urllib.request

SOUNDS_DIR = "/var/lib/asterisk/sounds"
PIPER_URL  = "http://127.0.0.1:5002/"
os.makedirs(SOUNDS_DIR, exist_ok=True)

try:
    with open("/var/lib/asterisk/agi-bin/menu.json", encoding="utf-8") as f:
        name = json.load(f).get("restaurant", {}).get("nom", "restaurant")
except Exception:
    name = "restaurant"

PHRASES = {
    "restaurant_greeting": f"Bonjour, {name}! Que souhaitez-vous commander?",
    "restaurant_silence":  "Pardon, je n'ai pas entendu. Pouvez-vous répéter?",
    "restaurant_timeout":  "Merci de votre appel. Au revoir!",
    "restaurant_error":    "Désolé, un problème est survenu. Veuillez rappeler.",
}

def piper_to_wav(text, wav_path):
    req = urllib.request.Request(
        PIPER_URL, data=text.encode("utf-8"),
        headers={"Content-Type": "text/plain; charset=utf-8"}
    )
    with urllib.request.urlopen(req, timeout=15) as resp:
        piper_data = resp.read()
    tmp = wav_path + ".piper.wav"
